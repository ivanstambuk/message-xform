Feature: Profile Routing & Header Injection
  ...
  Scenario: Test 7 — Header injection via profile routing (S-002-04)
    ...
    # Spec injects X-Transformed: true and X-Transform-Version: 1.0.0 into the
    # request headers. We verify by querying the echo backend directly to capture
    # the request headers it received (PA may strip backend response headers).
    ...
    When method POST
    Then status 200
    # The transform ran successfully (we got a 200)
    # Verify via response body that the echo received and returned the input
    And match response.key == 'value'

    # Verify header injection by checking PA logs for the transform event
    * def paLog = karate.exec('docker exec ' + paContainer + ' cat ' + paLogFile)
    * match paLog contains 'e2e-header-inject'
  ...
  Scenario: Test 8 — Multiple spec routing (S-002-09, S-002-15)
    ...
    # /api/headers/ → e2e-header-inject (verifies second spec is active)
    ...
    When method POST
    Then status 200
    # Verify body was passed through (identity JSLT: `.`)
    And match response.marker == 'routing-test'
