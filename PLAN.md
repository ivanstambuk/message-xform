...
- [ ] **Core byte-boundary refactor (Anti-Corruption Layer)** — Remove all Jackson types from core's public API. Core hides Jackson behind `byte[]`/`Map` boundary (ADR-0032), bundles and relocates its own copy. Enables multi-gateway support, independent security patching, and version freedom. Plan: `docs/research/core-byte-boundary-plan.md`.
