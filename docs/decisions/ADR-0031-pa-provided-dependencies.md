...
7. **Version-locked release strategy** — adapter version `9.0.x` targets
   PA `9.0.*`. This is standard practice in mature plugin ecosystems:

   | Ecosystem | Version Contract |
   |-----------|-----------------|
   | Elasticsearch plugins | Must declare `elasticsearch.version`; rejected at load if mismatched |
   ...
   2. Run `scripts/pa-extract-deps.sh` against it
   3. Review version diff — patch unchanged = compatible; minor = re-test;
      major = full adapter review
   4. Update `pa-provided.versions.toml` (auto-generated by step 2)
   5. Run full test suite (`./gradlew clean check`)
   6. Tag release `adapter-pingaccess-<PA_MAJOR>.<PA_MINOR>.<patch>`

8. **Runtime version guard** in `configure()` — warn if PA's runtime Jackson
   version differs from the adapter's compiled-against version. Compiled
   versions injected at build time via `processResources` into
   `META-INF/message-xform/pa-compiled-versions.properties`.

   Mismatch severity matrix:

   | Mismatch | Example | Risk | Action |
   |----------|---------|------|--------|
   ...
   | SLF4J 1.x→2.x | 1.7.36 → 2.0.x | Low | INFO log (expected) |
