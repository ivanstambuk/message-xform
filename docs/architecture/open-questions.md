...
|----|-------|----------|------------------------|--------|-------|-------|
| Q-033 | Ivan | **ProxyHandler TransformResult dispatch:** How should `ProxyHandler` consume the three `TransformResult` states (`SUCCESS`/`ERROR`/`PASSTHROUGH`)? Spec implies behaviour across FR-004-02/03/23 but no single FR explicitly maps the dispatch. | A: Add explicit FR with dispatch table; B: Expand FR-004-02/03 inline | Open | 2026-02-08 | severity: high; Feature 004 |
| Q-034 | Ivan | **wrapResponse data source:** `GatewayAdapter<R>` uses single type `R` for both `wrapRequest(R)` and `wrapResponse(R)`. For the proxy, `R=Context` works for requests, but the response comes from `UpstreamClient`'s `HttpResponse`, not `Context`. How to bridge? | A: Populate Javalin ctx with upstream response before calling `wrapResponse(ctx)`; B: Composite context type; C: Construct response `Message` directly in `ProxyHandler` | Open | 2026-02-08 | severity: high; Feature 004 |
