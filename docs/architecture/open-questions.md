...
|----|-------|----------|------------------------|--------|-------|-------|
| Q-039 | Ivan | CFG-004-16 `backend.max-body-bytes` is named as if backend-only, but FR-004-13 (Q-037) applies it bidirectionally (request + response). Should the config key be renamed to reflect its bidirectional nature? | A: Rename to `proxy.max-body-bytes` (top-level, clearly bidirectional). B: Rename to `max-body-bytes` (top-level). C: Keep `backend.max-body-bytes` — the name is established and documented; the bidirectional behaviour is noted in the spec text. | Open | 2026-02-08 | Env var `BACKEND_MAX_BODY_BYTES` also implies backend-only. If renamed, env var must change too. |
