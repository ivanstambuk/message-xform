...
- **Introduction:** In 2–4 sentences, explain what this feature or concept is, why it exists, and what goes wrong if we don't decide. No spec IDs, no config key names, no code — the reader should understand the problem before encountering technical detail.
...
- **Progressive disclosure (mandatory):** The Context section MUST follow the sub-bullet order defined in the template. Start with an introduction, then define terms, *then* cite spec references and technical detail. The reader should never encounter a technical term (e.g., "strict mode", "lenient mode", "pipeline chaining") that hasn't been introduced first. If a reader needs to ask "what is this concept?" after reading the Context, the card has failed.
