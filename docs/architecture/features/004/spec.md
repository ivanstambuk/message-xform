...
| ID | Requirement | Success path | Validation path | Failure path | Source |
|----|-------------|--------------|-----------------|--------------|--------|
| FR-004-06 | The proxy MUST implement `GatewayAdapter<Context>` where `Context` is Javalin's request context, providing `wrapRequest`, `wrapResponse`, and `applyChanges`. | `wrapRequest(ctx)` extracts: body → `MessageBody` (`MessageBody.json(...)` when JSON is present; `MessageBody.empty()` when absent), `headers` → `HttpHeaders` (lowercase keys with both single-value and multi-value views), `requestPath`, `requestMethod`, `queryString` (raw, without leading `?`), `statusCode` (`null` for requests), and `session` (`SessionContext.empty()` in standalone v1). Returns a fully populated `Message`. | Request with no body → `Message.body()` is `MessageBody.empty()` (per `Message` contract — body is never null). Response with no body (e.g., `204 No Content`) → `Message.body()` is `MessageBody.empty()`. | JSON parse error on request body → proxy returns `400 Bad Request` with RFC 9457 error. | SPI-001-04/05/06, ADR-0013. |
...
| FR-004-06b | `wrapResponse` MUST include the original request's path (`ctx.path()`) and method (`ctx.method()`) in the returned `Message`, since profile matching for response transforms operates on request criteria (Feature 001 API-001-01). `wrapResponse` MUST populate: body → `MessageBody`, `headers` → `HttpHeaders`, `statusCode`, `requestPath` (from original request), `requestMethod` (from original request), and `session` (`SessionContext.empty()` in standalone v1). `queryString` is set to `null` for responses. | `wrapResponse(ctx)` → `Message` with `requestPath = ctx.path()`, `requestMethod = ctx.method()`, upstream body/headers/status. | n/a | n/a | Feature 001 API-001-01. |
...
| FR-004-08 | `applyChanges` MUST write the transformed `Message` fields (body, headers, status code) back to the Javalin response context. **`applyChanges` is called only for RESPONSE SUCCESS** — request transforms flow directly to `UpstreamClient.forward(result.message())` without calling `applyChanges` (see FR-004-35 dispatch table). | Transformed body, headers, and status code are written to `ctx.result()`, `ctx.header()`, `ctx.status()`. | If the transformed `Message.body()` is `MessageBody.empty()`, `applyChanges` MUST set an empty response body. | n/a | SPI-001-06. |
| FR-004-09 | Header names MUST be normalized to lowercase in `wrapRequest` and `wrapResponse`, consistent with core engine conventions. This applies to both single-value and multi-value header views exposed via `HttpHeaders`. | `Content-Type: application/json` → `content-type: application/json` in `Message.headers().toSingleValueMap()` and `Message.headers().toMultiValueMap()`. | n/a | n/a | Feature 001 spec. |
...
| S-004-10 | **Request with no body:** `GET /api/users` matches a profile with request transform → transform receives `MessageBody.empty()` (serialized as JSON null to expression engine) → expression evaluates successfully → `SUCCESS` with expression output (not `PASSTHROUGH` — profile matched). |
...
| S-004-64 | **Response with no body (204):** Backend returns `204 No Content` → `wrapResponse` creates `Message` with `MessageBody.empty()` → profile matching proceeds normally → response returned to client. |
...
|-----|-------|--------------------------|
| ADR-0011 | Jackson `JsonNode` body representation | Proxy parses request/response JSON and wraps bytes in `MessageBody` for engine input |
