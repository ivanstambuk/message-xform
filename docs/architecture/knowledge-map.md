...
| `docs/operations/kubernetes-operations-guide.md` | K8s/k3s deployment ops: Helm chart patterns, init container JAR injection (mount at `/opt/out/instance/deploy` NOT staging), PingAM standalone Deployment, PA Admin API configuration (Site + Apps + MessageTransform rule), license/ConfigMap/Secret mounting, PA admin password override gotcha, troubleshooting table | Infrastructure (all K8s deployment), Feature 002 integration |
