...
| `docs/operations/kubernetes-operations-guide.md` | K8s/k3s deployment ops: Helm chart patterns, init container JAR injection (mount at `/opt/out/instance/deploy` NOT staging), PingAM standalone Deployment, PA Admin API configuration (Site + Apps + MessageTransform rule, password=`2Access`), license/ConfigMap/Secret mounting, Traefik IngressRoute + ServersTransport, E2E test validation section (port-forwarding, config overrides, header casing gotcha), troubleshooting table | Infrastructure (all K8s deployment), Feature 002 integration |
| `docs/operations/platform-deployment-guide.md` | 3-container platform (PA + AM + PD) deployment guide, K8s-first (Helm + standalone Deployment), TLS, WebAuthn import, message-xform plugin wiring (§9c), transform specs/profiles (am-auth-response-v2, am-webauthn-response, am-strip-internal; ADR-0036 body-predicate routing via match.when), troubleshooting. K8s: PD FQDN hostname verification gotcha, dual PD schema strategy. Docker Compose scripts archived to `scripts/legacy/`. | Infrastructure (all features requiring live IdP), Feature 002 integration |
| `deployments/platform/k8s/cloud-deployment-guide.md` | Cloud deployment guide for AKS, GKE, EKS. Container registry push (ACR, Artifact Registry, ECR), Helm values overlay pattern, cloud-native Ingress controllers, cert-manager + Let's Encrypt TLS, PingAM cloud adaptations, post-deployment verification. Plugin init container image replaces hostPath for cloud. | Infrastructure (cloud deployment) |
| `docs/operations/e2e-karate-operations-guide.md` | Gateway-neutral E2E test architecture, Docker lifecycle, Gradle integration, Karate patterns, IDE extension patches, K8s env (`lowerCaseResponseHeaders`, Traefik header casing P15, AM Host port P14) | All E2E modules, Feature 002, future gateway features |
...
| `docs/research/pingdirectory-cts-compatibility.md` | PingDirectory as CTS/Config/Policy store for PingAM — official vendor stance (PingDS only), codebase lineage (OpenDS → UnboundID/OpenDJ shared heritage), two tweaks (schema relaxation + etag mirror VA), verified test evidence, risk assessment. Unsupported but functional for dev/test. | Infrastructure (platform deployment), PingAM Operations Guide §6 |
| `deployments/platform/e2e/` | Standalone platform E2E tests (Karate JAR, no Gradle). Tests: `auth-login.feature` (3 scenarios), `auth-logout.feature` (1 scenario), `auth-passkey.feature` (3 scenarios: full reg+auth, device-exists auth, unsupported fallback), `auth-passkey-usernameless.feature` (2 scenarios: full reg+auth with UV+residentKey, discoverable credential entry point), `clean-url-login.feature` (3 scenarios), `clean-url-passkey.feature` (2 scenarios). Helpers: `webauthn.js` (pure JDK FIDO2 ceremony — EC P-256, CBOR, SHA256withECDSA; handles both auth and reg userVerification formats), `delete-device.feature` (AM Admin API device cleanup), `basic-auth.js` (HTTP basic auth helper). Config: `karate-config.js` (docker + k8s envs). Runner: `run-e2e.sh` (supports `--env k8s` with port-forwarding). Key gotchas: clear cookie jar between AM-direct and PA-proxied calls; ConfirmationCallback must stay at default value (not 0); usernameless userHandle must be base64(username); Traefik title-cases HTTP/1.1 headers (fix: lowerCaseResponseHeaders). | Phase 8 of platform PLAN |
